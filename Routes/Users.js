...
users.post("/addTransport", async (req, res) => {
  ...
  try {
    connect = await database.connection.getConnection();
    const [rows] = await connect.query(
      "INSERT INTO users_transport SET name = ?,description = ?,type = ?,max_weight = ?,user_id = ?,adr = ?,cubature = ?,transport_number = ?",
      [
        name,
        description,
        type,
...
users.post("/editTransport", async (req, res) => {
  ...
  try {
    connect = await database.connection.getConnection();
    const [rows] = await connect.query(
      "UPDATE users_transport SET name = ?,description = ?,type = ?,max_weight = ?,user_id = ?,adr = ?,cubature = ? ,transport_number = ? WHERE id = ?",
      [
        name,
        description,
        type,
  ...
  try {
    ...
    const [paymentUser] = await connect.query(
      `SELECT 
          ...
          COALESCE((SELECT SUM(amount_tir) FROM tir_balance_transaction  WHERE group_id IS NULL AND deleted = 0 AND user_id = ${userId} AND created_by_id = ${userId} AND transaction_type = 'subscription'), 0) AS tirgoBalance,
          COALESCE((SELECT SUM(amount_tir) FROM tir_balance_exchanges WHERE user_id = ${userId} AND balance_type = 'tirgo_service'), 0) - 
          COALESCE((SELECT SUM(amount_tir) FROM tir_balance_transaction  WHERE group_id IS NULL AND deleted = 0 AND user_id = ${userId} AND created_by_id = ${userId} AND transaction_type = 'service' AND status In(2, 3)), 0) AS serviceBalance;`
    );
    appData.status = true;
