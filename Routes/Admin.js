...
admin.post("/loginAdmin", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connect.query(
      "SELECT * FROM users_list WHERE username = ? AND password = ? AND user_type = 3 AND ban <> 1",
      [login, password]
    );
    if (rows.length) {
...
admin.post("/paid-way-transactions", async (req, res) => {
  ...
  try {
    ...
    const [driverTransports] = await connect.query(
      ...
      WHERE ut.active = 1 AND ul.agent_id <> 6820 AND ul.paid_way_kz = 1 AND ut.transport_number IS NOT NULL AND LENGTH(ut.transport_number) = 8 
